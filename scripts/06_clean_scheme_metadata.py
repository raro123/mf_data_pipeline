...
def compare_outputs(old_path, new_path):
    """
    Compare old and new outputs for validation.

    Returns True if all columns match exactly.
    """
    ...
    if set(old_df.columns) != set(new_df.columns):
        ...
        print(f"Old columns: {sorted(old_df.columns)}")
        print(f"New columns: {sorted(new_df.columns)}")
        return False

    all_match = True
    for col in sorted(old_df.columns):
        if col not in new_df.columns:
            print(f"ERROR: Column '{col}' missing in new output")
            all_match = False
            continue

        # Compare based on dtype
        if old_df[col].dtype == bool or new_df[col].dtype == bool:
            match = (old_df[col] == new_df[col]).all()
        elif old_df[col].dtype in ['object', 'category'] or new_df[col].dtype in ['object', 'category']:
            match = (old_df[col].fillna('') == new_df[col].fillna('')).all()
        elif pd.api.types.is_numeric_dtype(old_df[col]) and pd.api.types.is_numeric_dtype(new_df[col]):
            # For numeric, check null positions match and values are close
            null_match = (old_df[col].isna() == new_df[col].isna()).all()
            if null_match:
                value_match = ((old_df[col].fillna(0) - new_df[col].fillna(0)).abs() < 0.0001).all()
                match = null_match and value_match
            else:
                match = False
        else:
            match = (old_df[col] == new_df[col]).all()
        ...
        print(f"  {col}: {status} (old:{old_df[col].dtype}, new:{new_df[col].dtype})")

        if not match:
            all_match = False
            # Find first mismatch
            diff_mask = old_df[col] != new_df[col]
            if diff_mask.any():
                first_diff_idx = diff_mask.idxmax()
                print(f"    First mismatch at index {first_diff_idx}:")
                print(f"      Old: {old_df.loc[first_diff_idx, col]}")
                print(f"      New: {new_df.loc[first_diff_idx, col]}")

    if all_match:
        print("\nSUCCESS: All columns match!")
    else:
        print("\nERROR: Some columns do not match")
