# Mutual Fund Data Pipeline

Python data pipeline for collecting and preparing Indian mutual-fund data from
AMFI, with optional Zerodha instrument data. NIFTY benchmark data is maintained
by the separate `nifty_index_ingestion` repository and consumed downstream
through the datalake.

The repository currently uses a hybrid architecture:

- Cloudflare R2 is the operational store for scheduled NAV, metadata, AMC
  member, AUM, and TER jobs.
- Local CSV and Parquet files under `data/` support historical backfills,
  metadata cleaning, master-data maintenance, and exploratory analysis.
- DuckDB is used for R2 access, joins, validation queries, and Parquet output.
- Pandas is used for API response parsing and local transformations.
- GitHub Actions schedules the production jobs.

## Data Products

| Product | Purpose | Current destination |
| --- | --- | --- |
| Raw historical NAV | AMFI NAV history downloaded in date chunks | `data/raw/nav_historical/` and R2 |
| Raw daily NAV | Missing weekday NAV snapshots from AMFI | R2 `mutual_funds/raw/` |
| Canonical daily NAV | Enriched NAV and open-growth analysis tables | DuckLake `mf.nav_daily` and `mf.nav_daily_open_growth` |
| Scheme metadata | AMFI scheme attributes and plan classification | Local `data/processed/scheme_metadata/` |
| Scheme master data | All schemes ever observed, including inactive schemes | Local `data/processed/scheme_metadata/` |
| Raw AMC members | Weekly AMFI member, detail, and social records | R2 `mutual_funds/amc_members/` and DuckLake `mf.raw_amc_member_snapshot` |
| Scheme-wise AUM | Quarterly AMFI average AUM | Local dated Parquet and R2 `mutual_funds/aum/` |
| Raw TER | Monthly AMFI all-fund TER disclosure snapshots | R2 `mutual_funds/ter/` |

R2 paths are generated by `config.settings.R2` using this pattern:

```text
r2://financial-data-store/mutual_funds/<area>/<file>.parquet
```

## Current Automated Workflows

| Workflow | Schedule | Commands |
| --- | --- | --- |
| Daily NAV processing | Daily at 06:30 UTC / 12:00 IST | `fetch_daily_nav` |
| Scheme metadata extraction | Saturday at 00:30 UTC / 06:00 IST | `extract_scheme_metadata` |
| AMC member extraction | Saturday at 01:30 UTC / 07:00 IST | `extract_amc_members` |
| Scheme-wise AUM extraction | 10th of Jan/Apr/Jul/Oct at 00:30 UTC / 06:00 IST | `fetch_aum_data` |
| TER extraction | 5th and 20th at 07:30 UTC / 13:00 IST | `fetch_ter_data --scheduled` |

Every workflow also supports manual execution from the GitHub Actions tab.

## Setup

Python 3.9 or newer is recommended. The workflow uses Python 3.9 for NAV and
Python 3.12 for metadata, AUM, and TER extraction.

```bash
python -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
python -c "from config.settings import initialize_project; initialize_project()"
```

Create a local `.env` with the R2 credentials required by operational jobs:

```text
R2_ACCESS_KEY_ID=...
R2_SECRET_ACCESS_KEY=...
R2_ACCOUNT_ID=...
```

Optional API, retry, logging, and processing settings are documented in
`config/settings.py`.

## Running The Pipeline

Historical NAV backfill:

```bash
python -m scripts.fetch_historical_nav --start 20060101
python -m scripts.transform_historical_nav
```

Daily R2 NAV update:

```bash
python -m scripts.fetch_daily_nav
```

`fetch_daily_nav` derives its checkpoint from dated raw objects under
`mutual_funds/raw/nav_daily_*.parquet`; it does not depend on a clean NAV
output. Scheduled gap filling runs through yesterday so it does not request the
current day's NAV before publication. On an empty raw prefix, initialize
explicitly with:

```bash
python -m scripts.fetch_daily_nav --bootstrap-date YYYYMMDD
```

### Repairing NAVs reported late

Use the manual repair command when a dated daily raw snapshot already exists,
but AMFI subsequently publishes scheme/date observations that were absent from
that snapshot. Preview the repair before allowing an R2 write:

```bash
python -m scripts.repair_missing_nav --start YYYYMMDD --end YYYYMMDD
python -m scripts.repair_missing_nav --start YYYYMMDD --end YYYYMMDD --write
```

The command compares the current AMFI history with all existing raw NAV
objects. It writes only missing weekday `(scheme_code, date)` keys to a new,
immutable `nav_repair_missing_*.parquet` object. Existing keys with a changed
NAV are reported and skipped because the current clean table does not support
restatement precedence.

Do not use this command for today's pending AMFI publication, a missing daily
checkpoint, a full historical backfill, or a NAV restatement. After a repair
object is uploaded, manually run the datalake NAV ingestion:

```bash
gh workflow run mf-daily.yml --repo raro123/datalake
```

Repair filenames do not match the canonical `nav_daily_YYYYMMDD.parquet`
pattern, so they cannot advance the daily extraction watermark.

The datalake independently builds canonical `mf.nav_daily` and
`mf.nav_daily_open_growth` datasets from raw NAV files. The former
repository-side clean step is no longer scheduled or required.

Local scheme metadata and master-data refresh:

```bash
python -m scripts.clean_scheme_metadata
python -m scripts.build_scheme_masterdata
```

The local cleaner expects timestamped AMFI CSV files in
`data/raw/scheme_metadata/`. The scheduled `extract_scheme_metadata` job writes
raw Parquet directly to R2 instead, so these paths are not yet one continuous
workflow.

Weekly raw AMC member snapshot:

```bash
python -m scripts.extract_amc_members
```

The extractor stores one immutable, timestamped Parquet file under
`mutual_funds/amc_members/` and dispatches successful uploads to the datalake.
The raw rows preserve listing, detail, and social source records as JSON; no
clean AMC member table is built yet.

Optional jobs:

```bash
python -m scripts.fetch_aum_data
python -m scripts.fetch_aum_data --years 3
python -m scripts.ingest_zerodha_mf
```

Monthly TER extraction:

```bash
python -m scripts.fetch_ter_data --month YYYY-MM
python -m scripts.fetch_ter_data --start-month 2020-04 --end-month YYYY-MM
```

Scheduled mode fetches the previous month on days 1–10 and the current month
otherwise. TER snapshots are written as immutable Zstandard Parquet objects
under `mutual_funds/ter/`; the XLSX response is not retained. Successful runs
dispatch filename-idempotent raw ingestion to the datalake. Canonical TER
modelling remains deferred.

See `PIPELINE_EXECUTION_ORDER.md` for prerequisites and detailed ordering.

## Repository Layout

```text
config/                 Central paths, API settings, validation, and R2 setup
scripts/                Executable ingestion and transformation modules
utils/                  Shared NAV and logging helpers
.github/workflows/      Scheduled GitHub Actions jobs
data/                   Local raw, processed, and report artifacts (gitignored)
notebooks/              Exploratory analysis
```

## Important Current Limitations

- The R2 metadata extractor and the local metadata cleaner use different
  storage handoffs. The scheduled metadata workflow does not currently clean
  metadata or rebuild scheme master data.
- `transform_historical_nav.py` loads all historical CSV files into Pandas
  before writing Parquet. It is not currently a streaming or DuckDB-native
  transformation.
- The large local combined and analytical Parquet files are retained snapshots
  from an older pipeline generation; no current script rebuilds them.
- NAV freshness and completeness validation belongs with the datalake tables;
  the former report over the legacy clean Parquet has been retired.
- Unit coverage currently covers daily NAV checkpoint and gap behavior,
  missing-only repairs, AUM, AMC members, and TER extraction.
  `test_github_actions_setup.py` remains a separate environment and
  connectivity diagnostic rather than a unit test.

## Migration Status

The raw NAV fetcher is decoupled from the legacy clean NAV output, and the
legacy clean step has been removed from scheduled processing. See
`MIGRATION_STATUS.md` for the completed migration record and remaining R2
retirement steps.

## More Documentation

- `PIPELINE_EXECUTION_ORDER.md` - supported run order and dependencies
- `SCRIPT_INVENTORY.md` - purpose and status of every script
- `GITHUB_ACTIONS_SETUP.md` - GitHub secrets, schedules, and operations
- `MIGRATION_STATUS.md` - current datalake migration state and next actions
- `AGENTS.md` - engineering guidance for repository contributors and agents
